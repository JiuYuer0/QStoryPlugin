...
void sendQuestion(String stateKey, int questionIndex) {
    ...
    message.append("欢迎 请在300s内回答以下问题,验证过期永久封禁,回答错误永久封禁[").append(questionIndex + 1).append("/2]\n\n");
    ...
    message.append("\n非验证者回答会被`禁言`\n题目太难可擅用`DeepSeek`ChatGPT`\n题目太难可以发送`换题`更换题目[换题机会剩余");
...
void handleVerificationFailure(String stateKey, String reason) {
    ...
    String notice = "[AtQQ=" + state.userUin + "] 由于" + reason + "，未完成入群验证，已被永久封禁。";
    sendMsg(state.groupUin, "", notice);
    verifyStates.remove(stateKey);
}
